...
document.addEventListener("DOMContentLoaded", function () {
    // 1. Centralized Array of Products
    ...
    // 2. Select the target location container
    const gridTarget = document.getElementById("product-grid-target");

    // 3. Clear target element and execute template literal map loop
    if (gridTarget) {
        gridTarget.innerHTML = digitalProducts.map(product => `
            <article>
        ...
        `).join(''); // join('') joins array items into a clean continuous string
    }
});
